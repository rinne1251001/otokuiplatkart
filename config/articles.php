<?php

return [
    'list' => [
        [
            "title"         => "行くな(行け)平汝鉄路で乗り鉄【旅行記】【撮影地】",
            "desc"          => "まるで火星鉄道。公安とのふれあいもあるハートフル乗り鉄。おてがる撮影地もあるぞ",
            "category_name" => "海外の旅行記",
            "category"      => "foreign",
            "sub_category"  => "travelogue",
            "tags"          => "乗り鉄,撮り鉄,撮影地,中国,中国鉄路,平汝鉄路,SS3,公安,",
            "series"        => "china2026",
            "url"           => "rujigou_noritetsu",
            "img"           => "images/china2026/PKII2058.jpg",
            "date"          => "2026.04.08"
        ],
        [
            "title"         => "海外鉄入門１落単位②ホジケント線エレクトリーチカに乗る",
            "desc"          => "ホジケント線で乗り鉄",
            "category_name" => "海外の旅行記",
            "category"      => "foreign",
            "sub_category"  => "travelogue",
            "tags"          => "ウズベキスタン,エレクトリーチカ",
            "series"        => "uzkz",
            "url"           => "uzkz_2",
            "img"           => "images/uzbkkzqh2023/KIIP5333.jpg",
            "date"          => "2025.11.07"
        ],
        [
            "title"         => "神経衰弱",
            "desc"          => "旅先の写真で神経衰弱",
            "category_name" => "その他のこと",
            "category"      => "others",
            "sub_category"  => null,
            "tags"          => "ゲーム,写真",
            "series"        => null,
            "url"           => "memorygame",
            "img"           => "images/uzbkkzqh2023/mdm_004652_s.jpg",
            "date"          => "2025.10.06"
        ],
        [
            "title"         => "【撮り鉄】モンゴル国鉄　ホンホルΩカーブ行き方2023",
            "desc"          => "モンゴルで一番有名な撮影地へのアクセス方法※2023年版",
            "category_name" => "海外の鉄道",
            "category"      => "foreign",
            "sub_category"  => "travelogue,location",
            "tags"          => "モンゴル,海外の撮影地",
            "series"        => "honhol",
            "url"           => "honhol_spot",
            "img"           => "images/mngl2023/KIIP0740(3)_s.jpg",
            "date"          => "2025.04.20"
        ],
        [
            "title"         => "海外鉄入門１落単位①",
            "desc"          => "ウズベク入国。バザール見物と一瞬だけ撮り鉄",
            "category_name" => "海外の旅行記",
            "category"      => "foreign",
            "sub_category"  => "travelogue",
            "tags"          => "ウズベキスタン",
            "series"        => "uzkz",
            "url"           => "uzkz_1",
            "img"           => "images/uzbkkzqh2023/KIIP5143_s.jpg",
            "date"          => "2024.12.26"
        ],
        [
            "title"         => "海外鉄入門１落単位まとめ",
            "desc"          => "初めての海外旅行。撮り鉄としては０点。旅としては１２０点。",
            "category_name" => "海外の旅行記",
            "category"      => "foreign",
            "sub_category"  => "travelogue",
            "tags"          => "ウズベキスタン",
            "series"        => "uzkz",
            "url"           => "uzkz_matome",
            "img"           => "images/uzbkkzqh2023/mdm_004783_1.jpg",
            "date"          => "2024.12.26"
        ],
        [
            "title"         => "ギャラリー",
            "desc"          => "旅の写真をまとめました",
            "category_name" => "その他のこと",
            "category"      => "others",
            "sub_category"  => null,
            "tags"          => "写真",
            "series"        => null,
            "url"           => "album",
            "img"           => "images/album/1.jpg",
            "date"          => "2024.11.13"
        ],
    ],
    'series' => [
        [
            "name"  => "uzkz",
            "title" => "ウズベク＆カザフ",
            "desc"  => "ウズベキスタン＆カザフスタンのシリーズ",
        ],
        [
            "name"  => "honhol",
            "title" => "モンゴル",
            "desc"  => "モンゴルのシリーズ",
        ],
        [
            "name"  => "china2026",
            "title" => "中国",
            "desc"  => "中国旅行2026のシリーズ",
        ],
    ]
];